...
public class CellContextMenuWidget implements
	...
	public void foo() {
		...
		radioGroupItem.setValueMap("Select an existing constraint",
				"Add your own constraint", "no interpretation for this cell","I don't know");
		...
		// : Boolean, Integer, Double, VariabilityConceptRef, Partial, And, Or,
		// Xor, Multiple, Unknown, Empty, Inconsistent
		cbItem.setValueMap(Type.And.toString(), Type.Boolean.toString(),
				Type.Empty.toString(), Type.Integer.toString(),
				Type.Double.toString(), Type.VariabilityConceptRef.toString(),
				Type.Inconsistent.toString(), Type.Multiple.toString(),
				Type.Or.toString(), Type.Partial.toString(),
				Type.Unknown.toString(), Type.Xor.toString());
		...
		validate.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if ("Select an existing constraint".equals(radioGroupItem
						.getValue())) {
					ExperimentDataCellSingleton.getInstance(
							ExperimentDataCellSingleton.getSelectedCell()
									.get(0).getMatrixId()).setType(
							Type.valueOf(cbItem.getValueAsString()));
				} else if ("Add your own constraint".equals(radioGroupItem
						.getValue())) {
					ExperimentDataCellSingleton.getInstance(
							ExperimentDataCellSingleton.getSelectedCell()
									.get(0).getMatrixId()).setRemark(
							form.getValueAsString("OwnIntepretation"));
				}else if ("I don't know".equals(radioGroupItem
						.getValue())) {
					ExperimentDataCellSingleton.getInstance(
							ExperimentDataCellSingleton.getSelectedCell()
									.get(0).getMatrixId()).noIdea();
					
				
				}else {
					ExperimentDataCellSingleton.getInstance(
							ExperimentDataCellSingleton.getSelectedCell()
									.get(0).getMatrixId()).noInterpretation();
				}
